...
The repository includes an `andasy.hcl` configuration file that defines:
- Application name: `tartanvolt`
- Organization: `kkipngenokoech`

This configuration is used by the Andasy CLI during deployment.

## Workflow behavior

 - The workflow at `.github/workflows/deploy.yml` runs on pushes to `main`.
 - It builds the Vite/React application using `npm ci` and `npm run build`
 - The workflow requires a `package-lock.json` file (generated by `npm install`)
 - If you need to pass a specific app name or customize the deploy, either update the action usage or modify `andasy.hcl` with the desired configuration.
